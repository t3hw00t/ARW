...
- [Kernel] Supervisor Core Phase 2: finalize `RuntimeRegistry` adapter trait, lease-gated start/stop APIs, structured logs, and policy simulator coverage — in_progress (structured log stream with heartbeat dedupe plus `state_label`/`severity_label` landed; runtime start/stop now lease-gated via `runtime:manage`; policy simulator coverage landed with restore/shutdown tests).
- [Pack: Collaboration][Kernel] Launcher runtime panels: expose profiles, consent cues, and start/stop controls with keyboard parity; publish operator runbook excerpt in Launcher help cards — done (start/stop/restart controls with restart budgets plus embedded runbook excerpt ship in Launcher; consent cues, hardware recommendations, and accessibility overlays now land alongside refreshed keyboard/accessibility support).
...
- [Kernel] [t-250915090040-kern05] Migrate JSONL events journal to SQLite (remove old env) — done (journal tails kernel storage; ARW_EVENTS_JOURNAL removed)
...
- [Kernel] Asimov Capsule Guard (alpha)
  ...
  - [Kernel] [t-250916130002-asg02] Layered gating denies/contracts with lease sweeper + emergency teardown hook — done (lease sweeper + emergency teardown API/CLI shipped; Launcher UX polish remains)
  - [Kernel] [t-250916130003-asg03] Auto-replay verified capsules before actions/tools/egress/policy evaluation — done (proxy + logic unit entry points now refresh capsules; coverage via egress/policy regression tests)
- [Kernel] [t-250916130004-asg04] Admin UX + CLI for capsule presets, rotation, and audit trails — done (admin models UI now manages presets/audit; CLI adds preset/audit/trust commands)
...
- [Kernel] [t-250913001006-1003] Action Cache (MVP): wrap `tools_exec::run` with deterministic key (tool id, version, canonical JSON, env signature stub) and CAS’d outputs; Moka front with TTL; `tool.cache` events — done (bus tests cover hit/miss paths; CLI introspection via `arw-cli tools cache`).
...
- [Kernel] Policy: add network scopes (domain/IP/CIDR, port, protocol) and TTL leases; surface in UI — doing (structured scopes + TTL landed via `/egress/settings`; launcher scopes view + editing live; lease handoff next).
