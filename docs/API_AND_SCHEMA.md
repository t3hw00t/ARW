...
- `POST /admin/models/cas_gc` — CAS GC once `{ttl_hours, verbose?}`; emits `models.cas.gc` (response includes `deleted_items` when verbose).
- `GET  /admin/models/by-hash/:sha256` — serve a CAS blob by sha256 (egress-gated).
- `GET  /state/models_hashes` — list installed model hashes, sizes, providers, and referencing model IDs (`models`); supports `provider` and `model` filters. Payloads expose stable pagination hints (`prev_offset`, `next_offset`, `page`, `pages`, `last_offset`) for cursorless navigation.
