...
Missing MkDocs or Python deps? `mise run bootstrap:docs` (or `bash scripts/bootstrap_docs.sh`) hydrates a dedicated virtual environment at `.venv/docs` using the pinned stack from `requirements/docs.txt`. Activate it (`source .venv/docs/bin/activate`) when you need to install extras such as prompt compression helpers to keep your system Python pristine.
