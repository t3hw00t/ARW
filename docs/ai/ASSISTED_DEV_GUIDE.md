...
Need a lighter docs lint for fast feedback? Run `DOCS_CHECK_FAST=1 python3 scripts/docs_check.py` (or pass `--fast`) to skip the mkdocs build and deep scans; follow up with the full run before merging when time permits. Prefer tasks? `mise run docs:check` and `mise run docs:check:fast` wrap those helpers.
