...
- Persona-aware runs: the launcher now exposes a Persona card in Training Park. Pick the active persona once and every `/orchestrator/mini_agents/start_training` request carries `persona_id`, tagging the orchestrator job, summaries, and suggested logic units so downstream analytics stay aligned with the empathy stack. (Preview-only — follow the [Persona Preview Quickstart](persona_quickstart.md) before enabling.)
