...
| GET | `/state/episodes` | Episode rollups grouped by `corr_id` (duration/errors/kinds metadata + per-event payloads). Supports `?limit=`, `?project=` (slug or prefix), `?errors_only=`, `?kind_prefix=`, and `?since=` filters. | beta |
