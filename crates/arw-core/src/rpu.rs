...
pub fn verify_capsule(cap: &arw_protocol::GatingCapsule) -> bool {
    ...
    let issuer = match cap.issuer.as_deref() { Some(s) => s, None => return false };
    let entry = match trust.issuers.iter().find(|e| e.id == issuer) { Some(e) => e, None => return false };
    let sig_b64 = match &cap.signature { Some(s) => s, None => return false };
    let sig = match b64.decode(sig_b64.as_bytes()) { Ok(v) => v, Err(_) => return false };
    let msg = signing_bytes(cap);
    match entry.alg.as_str() {
        "ed25519" => {
            if sig.len() != 64 { return false; }
            let key = match b64.decode(entry.key_b64.as_bytes()) { Ok(v) => v, Err(_) => return false };
            if key.len() != 32 { return false; }
            let key_arr: [u8;32] = match key.as_slice().try_into() { Ok(a) => a, Err(_) => return false };
            let Ok(vk) = ed25519_dalek::VerifyingKey::from_bytes(&key_arr) else { return false };
            let sig_arr: [u8;64] = match sig.as_slice().try_into() { Ok(a) => a, Err(_) => return false };
        ...
        "secp256k1" => {
            ...
            let key = match b64.decode(entry.key_b64.as_bytes()) { Ok(v) => v, Err(_) => return false };
            let Ok(vk) = k256::ecdsa::VerifyingKey::from_sec1_bytes(&key) else { return false };
...
fn abac_allows(cap: &arw_protocol::GatingCapsule) -> bool {
    ...
    let now = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis() as u64;
    if cap.issued_at_ms > now.saturating_add(5 * 60 * 1000) { return false; } // future-dated too far
    if let Some(ttl) = cap.hop_ttl { if ttl == 0 { return false; } }
    if let Some(p) = &cap.propagate { if !matches!(p.as_str(), "none"|"children"|"peers"|"all") { return false; } }
    true
}
