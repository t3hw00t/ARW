...
pub enum WasiError {
    ...
    Runtime(String),
    #[error("denied: {reason}")]
    Denied { reason: String, dest_host: Option<String>, dest_port: Option<i64>, protocol: Option<String> },
}
...
impl ToolHost for NoopHost {
    async fn run_tool(&self, id: &str, _input: &Value) -> Result<Value, WasiError> { Err(WasiError::Unsupported(id.to_string())) }
}
    ...
    pub fn new() -> Result<Self> {
        ...
        if std::env::var("ARW_EGRESS_PROXY_ENABLE").ok().as_deref() == Some("1") {
            ...
            match reqwest::Proxy::all(&url) {
                Ok(p) => { cb = cb.proxy(p); }
                Err(e) => { return Err(anyhow::anyhow!(WasiError::Runtime(format!("proxy: {}", e)))); }
            }
        }
        let client = cb
            .build()
...
impl ToolHost for LocalHost {
    async fn run_tool(&self, id: &str, input: &Value) -> Result<Value, WasiError> {
        match id {
            "http.fetch" => {
                let url_s = input.get("url").and_then(|v| v.as_str()).ok_or_else(|| WasiError::Runtime("missing url".into()))?;
                ...
                if std::env::var("ARW_DNS_GUARD_ENABLE").ok().as_deref() == Some("1") {
                    ...
                    let is_dns_msg = input.get("headers").and_then(|v| v.as_object()).and_then(|h| h.get("accept").or_else(|| h.get("content-type"))).and_then(|v| v.as_str()).map(|s| s.contains("application/dns-message")).unwrap_or(false);
                    if doh_host || dot_port || is_doh_path || is_dns_msg {
                        return Err(WasiError::Denied { reason: "dns_guard".into(), dest_host: host.clone(), dest_port: port, protocol: Some(scheme) });
                    }
                }
                ...
                if std::env::var("ARW_EGRESS_BLOCK_IP_LITERALS").ok().as_deref() == Some("1") {
                    if let Some(h) = &host {
                        if h.parse::<std::net::IpAddr>().is_ok() {
                            return Err(WasiError::Denied { reason: "ip_literal".into(), dest_host: Some(h.clone()), dest_port: port, protocol: Some(scheme) });
                        }
                    }
                }
                if let Some(h) = &host {
                    if !self.host_allowed(h) {
                        return Err(WasiError::Denied { reason: "allowlist".into(), dest_host: Some(h.clone()), dest_port: port, protocol: Some(scheme) });
                    }
                }
                ...
                if let Some(cid) = input.get("connector_id").and_then(|v| v.as_str()) {
                    ...
                    let cpath = std::path::Path::new(&base).join("connectors").join(format!("{}.json", cid));
                    if let Ok(bytes) = std::fs::read(&cpath) {
                        if let Ok(v) = serde_json::from_slice::<serde_json::Value>(&bytes) {
                            auth_token = v.get("token").and_then(|x| x.as_str()).map(|s| s.to_string());
                            allowed_hosts_from_connector = v.get("meta").and_then(|m| m.get("allowed_hosts")).and_then(|arr| arr.as_array()).map(|arr| arr.iter().filter_map(|x| x.as_str().map(|s| s.to_string())).collect());
                            connector_provider = v.get("provider").and_then(|x| x.as_str()).map(|s| s.to_string());
                        }
                    }
                }
                ...
                let inferred_hosts: Option<Vec<String>> = if allowed_hosts_from_connector.is_none() {
                    match connector_provider.as_deref() {
                        ...
                        Some("microsoft") | Some("graph") => Some(vec!["graph.microsoft.com".into()]),
                        Some("dropbox") => Some(vec!["api.dropboxapi.com".into(), "content.dropboxapi.com".into()]),
                        Some("box") => Some(vec!["api.box.com".into()]),
                        _ => None,
                    }
                } else { None };
                let allow_hosts = allowed_hosts_from_connector.as_ref().or(inferred_hosts.as_ref());
                if let (Some(h), Some(allow)) = (&host, allow_hosts) {
                    ...
                    if !ok {
                        return Err(WasiError::Denied { reason: "connector_host".into(), dest_host: Some(h.clone()), dest_port: port, protocol: Some(scheme) });
                    }
                }
                let method = input.get("method").and_then(|v| v.as_str()).unwrap_or("GET").to_ascii_uppercase();
                let head_kb: usize = std::env::var("ARW_HTTP_BODY_HEAD_KB").ok().and_then(|s| s.parse().ok()).unwrap_or(64);
                let mut req = match method.as_str() {
                    ...
                    "POST" => {
                        ...
                        if let Some(body) = input.get("body").and_then(|v| v.as_str()) { rb = rb.body(body.to_string()); }
                        rb
                    }
                ...
                if let Some(tok) = &auth_token { req = req.header("authorization", format!("Bearer {}", tok)); }
                let resp = req.send().await.map_err(|e| WasiError::Runtime(e.to_string()))?;
                let status = resp.status().as_u16();
                let body = resp.bytes().await.map_err(|e| WasiError::Runtime(e.to_string()))?;
            ...
            "app.vscode.open" => {
                let p = input.get("path").and_then(|v| v.as_str()).ok_or_else(|| WasiError::Runtime("missing path".into()))?;
                let base = std::env::var("ARW_STATE_DIR").unwrap_or_else(|_| "state".into());
                let projects = std::path::Path::new(&base).join("projects");
                let full = if std::path::Path::new(p).is_absolute() { std::path::PathBuf::from(p) } else { projects.join(p) };
                let _ = std::fs::create_dir_all(full.parent().unwrap_or(&projects));
                match std::process::Command::new("code").arg(full.as_os_str()).spawn() {
            ...
            "fs.patch" => {
                ...
                let path_s = input.get("path").and_then(|v| v.as_str()).ok_or_else(|| WasiError::Runtime("missing path".into()))?;
                let content = input.get("content").and_then(|v| v.as_str()).unwrap_or("").as_bytes().to_vec();
                let pre = input.get("pre_sha256").and_then(|v| v.as_str());
                let mkdirs = input.get("mkdirs").and_then(|v| v.as_bool()).unwrap_or(true);
                ...
                let target = if full.is_absolute() { full.to_path_buf() } else { base_path.join(full) };
                let _canon_base = std::fs::canonicalize(&base_path).unwrap_or(base_path.clone());
                let parent = target.parent().ok_or_else(|| WasiError::Runtime("bad path".into()))?.to_path_buf();
                if mkdirs { std::fs::create_dir_all(&parent).map_err(|e| WasiError::Runtime(e.to_string()))?; }
