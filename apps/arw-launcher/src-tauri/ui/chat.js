...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (CAN_CAPTURE) {
    ...
    try{
      ...
      const saveBtn=document.createElement('button'); saveBtn.className='ghost'; saveBtn.textContent='Save to project'; saveBtn.addEventListener('click', async ()=>{ if (out && out.path){ const res = await ARW.saveToProjectPrompt(out.path); if (res) await ARW.maybeAppendToNotes(res.proj, res.dest, out.path); } });
    ...
    try{
      ...
      const saveBtn2=document.createElement('button'); saveBtn2.className='ghost'; saveBtn2.textContent='Save to project'; saveBtn2.addEventListener('click', async ()=>{ if (out && out.path){ const res = await ARW.saveToProjectPrompt(out.path); if (res) await ARW.maybeAppendToNotes(res.proj, res.dest, out.path); } });
    ...
    try{
      ...
      const saveBtn3=document.createElement('button'); saveBtn3.className='ghost'; saveBtn3.textContent='Save to project'; saveBtn3.addEventListener('click', async ()=>{ if (out && out.path){ const res = await ARW.saveToProjectPrompt(out.path); if (res) await ARW.maybeAppendToNotes(res.proj, res.dest, out.path); } });
