    ...
    // Menu callbacks: use global MenuEvent receiver
    {
        ...
        thread::spawn(move || loop {
            if let Ok(ev) = MenuEvent::receiver().recv() {
                if ev.id == start {
                    let _ = start_service_inner(&svc, port);
                }
                if ev.id == stop {
                    let _ = stop_service_inner(&svc, port);
                }
                if ev.id == open {
                    let _ = open::that(format!("http://127.0.0.1:{}/debug", port));
                }
                if ev.id == quit {
                    let _ = stop_service_inner(&svc, port);
                    std::process::exit(0);
