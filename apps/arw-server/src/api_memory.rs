...
#[derive(Deserialize)]
pub(crate) struct MemPutReq { pub lane: String, #[serde(default)] pub kind: Option<String>, #[serde(default)] pub key: Option<String>, pub value: Value, #[serde(default)] pub embed: Option<Vec<f32>>, #[serde(default)] pub tags: Option<Vec<String>>, #[serde(default)] pub score: Option<f64>, #[serde(default)] pub prob: Option<f64> }
pub async fn memory_put(State(state): State<AppState>, Json(req): Json<MemPutReq>) -> impl IntoResponse {
...
pub async fn state_memory_select(State(state): State<AppState>, Query(q): Query<std::collections::HashMap<String, String>>) -> impl IntoResponse {
...
#[derive(Deserialize)]
pub(crate) struct MemEmbedReq { pub embed: Vec<f32>, #[serde(default)] pub lane: Option<String>, #[serde(default)] pub limit: Option<i64> }
pub async fn memory_search_embed(State(state): State<AppState>, Json(req): Json<MemEmbedReq>) -> impl IntoResponse {
...
#[derive(Deserialize)]
pub(crate) struct MemHybridReq { #[serde(default)] pub q: Option<String>, #[serde(default)] pub embed: Option<Vec<f32>>, #[serde(default)] pub lane: Option<String>, #[serde(default)] pub limit: Option<i64> }
pub async fn memory_select_hybrid(State(state): State<AppState>, Json(req): Json<MemHybridReq>) -> impl IntoResponse {
    let lane_opt = req.lane.as_deref();
    let limit = req.limit.unwrap_or(20);
...
#[derive(Deserialize)]
pub(crate) struct MemCoherentReq { #[serde(default)] pub q: Option<String>, #[serde(default)] pub embed: Option<Vec<f32>>, #[serde(default)] pub lane: Option<String>, #[serde(default)] pub limit: Option<i64>, #[serde(default)] pub expand_per_seed: Option<i64> }
pub async fn memory_select_coherent(State(state): State<AppState>, Json(req): Json<MemCoherentReq>) -> impl IntoResponse {
    let lane_opt = req.lane.as_deref();
...
pub async fn state_memory_recent(State(state): State<AppState>, Query(q): Query<std::collections::HashMap<String, String>>) -> impl IntoResponse {
    let lane = q.get("lane").map(|s| s.as_str());
...
#[derive(Deserialize)]
pub(crate) struct MemLinkReq { pub src_id: String, pub dst_id: String, #[serde(default)] pub rel: Option<String>, #[serde(default)] pub weight: Option<f64> }
pub async fn memory_link_put(State(state): State<AppState>, Json(req): Json<MemLinkReq>) -> impl IntoResponse {
...
pub async fn state_memory_links(State(state): State<AppState>, Query(q): Query<std::collections::HashMap<String, String>>) -> impl IntoResponse {
...
pub async fn memory_explain_coherent(State(state): State<AppState>, Json(req): Json<MemCoherentReq>) -> impl IntoResponse {
    let lane_opt = req.lane.as_deref();
    let limit = req.limit.unwrap_or(30);
    let expand_n = req.expand_per_seed.unwrap_or(3).max(0).min(10);
    let seeds = match state.kernel.select_memory_hybrid(req.q.as_deref(), req.embed.as_deref().map(|v| v.as_ref()), lane_opt, (limit/2).max(1)) {
    ...
    for mut it in seeds.clone() {
        ...
        if let Some(obj) = it.as_object_mut() { obj.insert("cscore".into(), json!(cscore)); obj.insert("explain".into(), json!({"sim": sim, "fts": fts, "recency": recency, "utility": util})); }
    ...
    for it in seeds.iter() {
        ...
        if let Ok(links) = state.kernel.list_memory_links(id, expand_n) {
            for lk in links {
                let dst_id = lk.get("dst_id").and_then(|v| v.as_str()).unwrap_or("");
                if dst_id.is_empty() || seen.contains(dst_id) { continue; }
                if let Ok(Some(mut rec)) = state.kernel.get_memory(dst_id) {
                    let weight = lk.get("weight").and_then(|v| v.as_f64()).unwrap_or(1.0) as f32;
                    let recency = rec.get("updated").and_then(|v| v.as_str()).and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok()).map(|t| {
                        let age = now.signed_duration_since(t.with_timezone(&chrono::Utc)).num_seconds().max(0) as f64;
                        let hl = 3600f64 * 6f64; ((-age/hl).exp()) as f32
                    }).unwrap_or(0.5);
                    let base = it.get("cscore").and_then(|v| v.as_f64()).unwrap_or(0.3) as f32;
                    let cscore = 0.6*base + 0.25*weight + 0.15*recency;
                    if let Some(obj) = rec.as_object_mut() { obj.insert("cscore".into(), json!(cscore)); obj.insert("explain".into(), json!({"base": base, "link_weight": weight, "recency": recency})); }
                    seen.insert(dst_id.to_string());
                    scored.push((cscore, rec));
                }
            }
    ...
    (axum::http::StatusCode::OK, Json(json!({"items": items, "mode": "coherent", "explain": true}))).into_response()
}
