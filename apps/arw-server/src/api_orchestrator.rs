...
pub async fn orchestrator_start_training(
    ...
    let id = match state
        .kernel
        .insert_orchestrator_job(&goal, req.data.as_ref())
    {
        Ok(id) => id,
        Err(e) => {
            return (
    ...
    tokio::spawn(async move {
        ...
        for i in 1..=steps {
            ...
            let _ = state2.kernel.update_orchestrator_job(
                &id_clone,
                Some(if i < steps { "running" } else { "completed" }),
                Some(p),
            );
            state2.bus.publish(
