...
pub(crate) struct ApplyReq {
    ...
    #[serde(default)] pub schema_ref: Option<String>,
    #[serde(default)] pub schema_pointer: Option<String>,
}

#[derive(Deserialize, Clone)]
pub(crate) struct PatchOp { pub target: String, pub op: String, pub value: Value }
...
pub(crate) fn ensure_path<'a>(root: &'a mut Value, path: &str) -> &'a mut Value {
    ...
    for seg in path.split('.') {
        ...
        if !map.contains_key(seg) { map.insert(seg.to_string(), json!({})); }
...
pub(crate) fn get_by_dot<'a>(root: &'a Value, path: &str) -> Option<&'a Value> {
    ...
    for seg in path.split('.') {
        ...
        match cur {
            Value::Object(map) => {
                if let Some(v) = map.get(seg) { cur = v; } else { return None; }
            }
            _ => return None,
        }
    }
...
pub(crate) fn infer_schema_for_target(target: &str) -> Option<(String, String)> {
    fn load_map() -> Option<serde_json::Value> {
        let path = std::env::var("ARW_SCHEMA_MAP").ok().unwrap_or_else(|| "configs/schema_map.json".into());
        let p = std::path::Path::new(&path);
        if p.exists() { if let Ok(bytes) = std::fs::read(p) { return serde_json::from_slice::<serde_json::Value>(&bytes).ok(); } }
        None
    }
        ...
        if let Some(obj) = map.get(seg) {
            if let Some(schema_ref) = obj.get("schema_ref").and_then(|v| v.as_str()) {
                let pointer = if let Some(pp) = obj.get("pointer_prefix").and_then(|v| v.as_str()) {
                    if target.len() > seg.len() { format!("{}{}", pp, &target[seg.len()..]) } else { pp.to_string() }
                } else { target.to_string() };
                if std::path::Path::new(schema_ref).exists() { return Some((schema_ref.to_string(), pointer)); }
            }
        }
    }
    let (schema_file, pointer) = match seg {
        "recipes" => ("spec/schemas/recipe_manifest.json", target.to_string()),
        "policy" | "policy_network" | "network" => ("spec/schemas/policy_network_scopes.json", target.to_string()),
        _ => return None,
    };
    if std::path::Path::new(schema_file).exists() { Some((schema_file.to_string(), pointer)) } else { None }
}
...
pub(crate) fn validate_patch_value(v: &Value) -> Result<(), Vec<Value>> {
    ...
    match res {
        Err(errors) => Err(errors.map(|e| json!({"path": e.instance_path.to_string(), "error": e.to_string()})).collect()),
        Ok(_) => Ok(())
    }
}

pub async fn patch_apply(State(state): State<AppState>, headers: HeaderMap, Json(req): Json<ApplyReq>) -> impl IntoResponse {
    if !admin_ok(&headers) {
    ...
    for op in &req.patches {
        if op.op == "merge" || op.op == "set" {
            ...
            unsafe {
                ...
                if op.op == "merge" { merge_values(dst, &op.value); } else { *dst = op.value.clone(); }
    ...
    if schema_opt.is_none() {
        if let Some(first) = req.patches.first() { schema_opt = infer_schema_for_target(&first.target).map(|(p,ptr)| { pointer_opt = Some(ptr); p }); }
    }
    ...
    if !dry {
        let snapshot_id = match state.kernel.insert_config_snapshot(&cfg) { Ok(id) => id, Err(e) => return (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"type":"about:blank","title":"Error","status":500, "detail": e.to_string()}))) };
        { let mut hist = state.config_history.lock().await; hist.push((snapshot_id.clone(), cfg.clone())); }
        { let mut cur = state.config_state.lock().await; *cur = cfg.clone(); }
        state.bus.publish("config.patch.applied", &json!({"ops": req.patches.len(), "snapshot_id": snapshot_id}));
        let json_patch: Vec<Value> = diffs.iter().filter_map(|d| {
            let p = d.get("pointer").and_then(|v| v.as_str())?; let v = d.get("after").cloned().unwrap_or(json!({})); Some(json!({"op":"replace","path": p, "value": v}))
        }).collect();
        return (axum::http::StatusCode::OK, Json(json!({"ok": true, "dry_run": false, "config": cfg, "diff_summary": diffs, "json_patch": json_patch })));
    }
    ...
    (axum::http::StatusCode::OK, Json(json!({"ok": true, "dry_run": true, "config": cfg, "diff_summary": diffs, "json_patch": json_patch })))
}

#[derive(Deserialize)]
pub(crate) struct RevertReq { pub snapshot_id: String }

pub async fn patch_revert(State(state): State<AppState>, headers: HeaderMap, Json(req): Json<RevertReq>) -> impl IntoResponse {
    if !admin_ok(&headers) {
        return (axum::http::StatusCode::UNAUTHORIZED, Json(json!({"type":"about:blank","title":"Unauthorized","status":401})));
    }
    let mut hist = state.config_history.lock().await;
    if let Some((_, snap)) = hist.iter().rev().find(|(id, _)| id == &req.snapshot_id).cloned() {
        let new_id = uuid::Uuid::new_v4().to_string();
        { let mut cur = state.config_state.lock().await; *cur = snap.clone(); }
        hist.push((new_id.clone(), snap.clone()));
        state.bus.publish("logic.unit.reverted", &json!({"snapshot_id": req.snapshot_id, "new_snapshot_id": new_id}));
        (axum::http::StatusCode::OK, Json(json!({"ok": true, "snapshot_id": new_id, "config": snap})))
    } else {
        (axum::http::StatusCode::NOT_FOUND, Json(json!({"type":"about:blank","title":"Not Found","status":404})))
    }
}

pub async fn state_config_snapshots(State(state): State<AppState>, Query(q): Query<std::collections::HashMap<String, String>>) -> impl IntoResponse {
    let limit = q.get("limit").and_then(|s| s.parse::<i64>().ok()).unwrap_or(50);
    let items = state.kernel.list_config_snapshots(limit).unwrap_or_default();
    (axum::http::StatusCode::OK, Json(json!({"items": items}))).into_response()
}

pub async fn state_config_snapshot_get(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    match state.kernel.get_config_snapshot(&id) {
        Ok(Some(cfg)) => (axum::http::StatusCode::OK, Json(cfg)).into_response(),
        Ok(None) => (axum::http::StatusCode::NOT_FOUND, Json(json!({"type":"about:blank","title":"Not Found","status":404}))).into_response(),
...
#[derive(Deserialize)]
pub(crate) struct ValidateReq { pub schema_ref: String, #[serde(default)] pub schema_pointer: Option<String>, #[serde(default)] pub config: Option<Value> }
...
pub async fn state_schema_map() -> impl IntoResponse {
    ...
    if p.exists() {
        ...
        (axum::http::StatusCode::OK, Json(json!({"path": path, "map": json!({})}))).into_response()
    }
}

#[derive(Deserialize)]
pub(crate) struct InferReq { pub target: String }
