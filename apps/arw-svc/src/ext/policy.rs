...
pub async fn allow_apply(action: &str, params: &Value) -> Result<(), String> {
    // Per-hour cap (very small by default)
    {
        ...
        let cap: u32 = std::env::var("ARW_FEEDBACK_APPLY_PER_HOUR").ok().and_then(|s| s.parse().ok()).unwrap_or(3);
        if bw.count >= cap { return Err("rate limit reached".into()); }
        // don't increment yet; only if bounds pass
    }

    // Bounds per action
    let (http_min, http_max) = {
        let min = std::env::var("ARW_FEEDBACK_HTTP_TIMEOUT_MIN").ok().and_then(|s| s.parse().ok()).unwrap_or(5u64);
        let max = std::env::var("ARW_FEEDBACK_HTTP_TIMEOUT_MAX").ok().and_then(|s| s.parse().ok()).unwrap_or(300u64);
        (min, max)
    };
    let (mem_min, mem_max) = {
        let min = std::env::var("ARW_FEEDBACK_MEM_LIMIT_MIN").ok().and_then(|s| s.parse().ok()).unwrap_or(50u64);
        let max = std::env::var("ARW_FEEDBACK_MEM_LIMIT_MAX").ok().and_then(|s| s.parse().ok()).unwrap_or(2000u64);
        (min, max)
    };
