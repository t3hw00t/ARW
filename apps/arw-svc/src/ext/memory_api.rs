...
#[arw_admin(
    method = "POST",
    path = "/admin/memory/save",
    summary = "Save memory to disk"
)]
...
pub(crate) async fn memory_save(State(state): State<AppState>) -> impl IntoResponse {
    if let Some(svc) = state.resources.get::<MemoryService>() {
        match svc.save().await {
            ...
            Err(e) => return super::ApiError::internal(&e).into_response(),
        }
    }
...
#[arw_admin(
    method = "POST",
    path = "/admin/memory/load",
    summary = "Load memory from disk"
)]
#[arw_gate("memory:load")]
pub(crate) async fn memory_load(State(state): State<AppState>) -> impl IntoResponse {
    if let Some(svc) = state.resources.get::<MemoryService>() {
        match svc.load().await {
            Ok(v) => return super::ok::<serde_json::Value>(v).into_response(),
            Err(e) => return super::ApiError::not_found(&e).into_response(),
        }
    }
...
pub(crate) async fn memory_limit_get(State(state): State<AppState>) -> impl IntoResponse {
    if let Some(svc) = state.resources.get::<MemoryService>() {
        return super::ok(serde_json::json!({ "limit": svc.get_limit().await })).into_response();
    }
...
#[arw_admin(
    method = "POST",
    path = "/admin/memory/limit",
    summary = "Set memory limit"
)]
#[arw_gate("memory:limit:set")]
pub(crate) async fn memory_limit_set(
    State(state): State<AppState>,
    Json(req): Json<super::SetLimit>,
) -> impl IntoResponse {
    if let Some(svc) = state.resources.get::<MemoryService>() {
        svc.set_limit(req.limit).await;
