...
#[derive(Deserialize)]
pub(crate) struct SetLimit { pub limit: usize }
...
pub(crate) async fn memory_load() -> impl IntoResponse {
    match load_json_file_async(&memory_path()).await {
        ...
        None => (axum::http::StatusCode::NOT_FOUND, Json(serde_json::json!({"ok": false, "error":"no memory.json"}))).into_response(),
    }
}
...
pub(crate) async fn memory_apply(State(state): State<AppState>, Json(req): Json<ApplyMemory>) -> impl IntoResponse {
    ...
    if let Some(arr) = lane {
        ...
        (axum::http::StatusCode::ACCEPTED, Json(serde_json::json!({"ok": true}))).into_response()
    } else {
        (axum::http::StatusCode::BAD_REQUEST, Json(serde_json::json!({"ok": false, "error": "invalid kind"}))).into_response()
    }
}
