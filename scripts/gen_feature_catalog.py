...
def render_feature(feature: dict) -> str:
    ...
    if ssot:
        ...
        if paths:
            lines.append(f"  _Source_: {join_backtick(paths, limit=3)}")
    return "\n".join(lines)
...
def render_catalog(catalog: dict, features: dict) -> str:
    ...
    out.append(
        ...
        "It extends the shared language in `docs/developer/style.md` and draws from `interfaces/features.json` and "
        "`interfaces/feature_catalog.json`."
    )
    out.append("\n")
