...
main_menu() {
  ...
  11) Generate systemd user service
  ...
  24) Generate reverse proxy templates (nginx/caddy)
  25) Start Caddy reverse proxy (https://localhost:8443)
  26) Stop Caddy reverse proxy
  26a) Start Caddy with selected Caddyfile
  27) Start Nginx reverse proxy (http://localhost:8080)
  28) Stop Nginx reverse proxy
  29) Disable debug now
  30) TLS wizard (LE/mkcert/self-signed)
...
EOF
    ...
    case "$pick" in
      ...
      22) ic_security_tips ;;
      23) troubleshoot ;;
      24) reverse_proxy_templates ;;
      25) reverse_proxy_caddy_start ;;
      26) reverse_proxy_caddy_stop ;;
      26a) reverse_proxy_caddy_choose_start ;;
      27) reverse_proxy_nginx_start ;;
      28) reverse_proxy_nginx_stop ;;
      29) DEBUG=0; unset ARW_DEBUG; ic_info "Debug disabled for this session." ;;
      30) tls_wizard ;;
      31) session_summary ;;
      32) stop_all ;;
...
[Service]
Environment=ARW_PORT=$PORT
Environment=ARW_CONFIG=${CFG_PATH}
Environment=ARW_DOCS_URL=${DOCS_URL}
Environment=ARW_ADMIN_TOKEN=${ADMIN_TOKEN}
ExecStart=$svc
Restart=on-failure
...
UNIT
  ic_info "Wrote $unit"
  echo "Run: systemctl --user daemon-reload && systemctl --user enable --now arw.service"
  ic_press_enter
}
