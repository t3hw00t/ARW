...
# Ensure local bin in PATH for on-the-fly installs (jq, etc.)
...
configure_settings() {
  ...
  if [[ -z "${ADMIN_TOKEN}" ]]; then
    read -r -p "Generate admin token now? (Y/n): " gen; if [[ "${gen,,}" != n* ]]; then
      ...
      ic_warn "Store this token in a password manager or .env file you control."
    fi
  fi
...
do_docs() {
  if [[ $BUILD_DOCS -eq 1 ]]; then
    ...
    # If local venv exists, add to PATH for mkdocs
...
nats_guidance() {
  ...
  echo "   - Homebrew (Linux/macOS): brew install nats-server"
  ...
  if [[ "${ARW_ALLOW_SYSTEM_PKGS:-0}" == "1" ]]; then
    ...
    case "$pm" in
      brew) ic_info "Attempting: brew install nats-server"; brew install nats-server || true ;;
      *) ic_warn "Use Docker or manual download on this platform." ;;
    esac
  else
    ic_warn "System package managers are disabled. Enable in Dependencies to try brew/apt."
  fi
  ic_press_enter
}
...
EOF
  ...
  ic_section "Next"
  printf "  Start the service with: %s\n" "$(_ic_color bold "$DIR/interactive-start-linux.sh")"
}
